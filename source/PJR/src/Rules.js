  ...
  return (
    <div className={classes.component}>
      ...
      <div className={classes.cards}>
        {
          cards.map(c => <Card card={c} value={c.value} type={c.type}/>)
        }
      </div>
    </div>
  );
...
function Three({classes, settings: {lang}}) {
  const cards = [{type: 'V', from: 1, to: 15}, {type: 'J', from: 2, to: 14}, {type: 'O', from: 3, to: 13}, {type: 'R', from: 7, to: 9}];
  ...
  return (
    <div className={classes.component}>
      ...
      {
        cards.map(c => <div className={classes.cardsInfo}>{lang.misc.from} {c.from} {lang.misc.to} {c.to}</div>)
      }
      </div>
    ...
    return (
      <BackgroundTheme settings={settings} className={classes.page}>
        <div className={classes.header}>
          <IconButton className={classes.back} color="white" aria-label="Menu" component={Link} to='/'>
              <ArrowBack />
          </IconButton>
        </div>
