...
class Game extends EventEmitter {
  constructor(server, pseudo) {
    ...
    /** @private */ this.currentPlayerIndex = -1; // Current player index (from 0 to playerCount-1)
    ...
    /** @private */ this.lastCastPlayed = null;   // Last card played
   ...
   */
  addToHerd(card) {
    if(Array.isArray(this.herd) && this.herd.length === 0) {
      this.herd.push(card);
    }else if(card.type !== card.SPECIAL || card.value === '0' || card.value === '16') {
      ...
      if(card.value.startsWith('R')) {
        let prevCard = null;
        this.herd
          .every((herdCard, i) => {
            if(herdCard.type === herdCard.SPECIAL)
              prevCard = null;
  ...
  getPlayableCards() {
    return this.hand.filter(card => {
      ...
      if(card.type !== card.SPECIAL || card.value === '0' || card.value === '16') {
      ...
      if(card.value.endsWith('A')) {
        return this.herd.find((herdCard, i) =>
          herdCard.type !== herdCard.SPECIAL
            && parseInt(card.value) === parseInt(herdCard.value));
      }
  ...
  drawCard(card, gameId, roundId, turnId) {
    if(this.verify(gameId, roundId, turnId)) {
      this.hand.splice(this.hand.indexOf(this.lastCastPlayed), 1);
      if(card) {
        this.hand.push(card);
        this.emit('handUpdate', [...this.hand]);
      }
...
class GameNetworkInterface extends EventEmitter {
  ...
  router(message) {
    if(this.readyState === GameNetworkInterface.CONNECTING)
      ...
      messageRouter(message)
        ...
        .route('IE-<action:string>-<game id:int>-<round id:int>-<turn id:int>', console.log)
   ...
   */
  endRound(info) {
    let scores = info.scores.split(',').map(parseInt),
      totalScores = info.totalScores.split(',').map(parseInt);
...
Card.prototype.GREEN = 'V';
Card.prototype.YELLOW = 'J';
Card.prototype.ORANGE = 'O';
Card.prototype.RED = 'R';
Card.prototype.SPECIAL = 'S';

Card.FLYCOUNT = {
  [Card.prototype.GREEN]: 0,
  [Card.prototype.YELLOW]: 1,
  [Card.prototype.ORANGE]: 2,
  [Card.prototype.RED]: 3,
  [Card.prototype.SPECIAL]: 5
}