...
public class UIManager {
    ...
    private void activate(String name) {
    	Pane pane = screenMap.get(currentLang + name);
        scene.setRoot(pane);
    }
    ...
    private void add(String newFXML, String oldFXML) {
    	...
    	Pane pane = screenMap.get(currentLang + newFXML);
   		...
   		ComputeAnimation.playFadeTransition(pane, .75, 1);
   		scene.setRoot(g);
    }
    ...
    public void removeToScene() {
    	...
    	p.setOpacity(0);
   		g.getChildren().add(p);
    	ComputeAnimation.playFadeTransition(p, .75, 1);
    	scene.setRoot(g);
    }
    ...
    // --- Start Leave
    public void exit() {
    	EventManager.getInstance().shutDownPCJ();
    }    
