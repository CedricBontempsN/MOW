...
public class AnalyseServeurTCP  extends AnalyseTCP implements TCPListener{
	...
	public void messageRecuTcp(String message,int portClient) {
		...
		if(message.equals("GET_LATENCY")) {
			...
			case "CP": 	notificationRejoindrePartie(valueData,""+portClient);
				break;
			case "RT": 	notificationJoueurTroupeau(valueData);
				break;
			case "JC": 	notificationJoueurCarte(valueData);
				break;
			case "ISJ": notificationJoueurSens(valueData);
				break;
			default: 	
				break;
		}
	...
	private void notificationJoueurTroupeau(ArrayList<String> valueData) {
			igca.joueurRentreTroupeau(valueData.get(1),valueData.get(2),valueData.get(3),valueData.get(4));
	}
	
	private void notificationJoueurCarte(ArrayList<String> valueData) {
		igca.joueurAJouerCarte(valueData.get(1),valueData.get(2),valueData.get(3),valueData.get(4)); 
	}

	private void notificationJoueurSens(ArrayList<String> valueData) {
		igca.joueurInverseSensJeu(valueData.get(1),valueData.get(2),valueData.get(3),valueData.get(4));
	}
